...
def split_CUHK_PEDE():
    root_dir = '/root/autodl-tmp/PSPD/dataset/CUHK-PEDES'
...
class cuhk_caption_gen_train(Dataset):
    def __init__(self, transform, image_root, max_words=72, prompt=''):
        train_list, _, _ = split_CUHK_PEDE()
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(image_path,ann['file_path'])
...
class cuhk_caption_train(Dataset):
    def __init__(self, transform, image_root, max_words=72, prompt=''):
        train_list, _, _ = split_CUHK_PEDE()
        self.annotation = train_list
        self.transform = transform
...
class pre_cuhk_pede_train(Dataset):
    def __init__(self, transform, image_root, max_words=72, prompt=''):

        output_file = './pre_gen_data/cuhk_gen_pre_caption.json'
...
class cuhk_pre_train(Dataset):
    def __init__(self, transform, image_root, max_words=72, prompt=''):
        '''
        image_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
        '''
        # self.train_list, _, _ = split_CUHK_PEDE()
        output_file = "./gen_data/cuhk_gen_caption.json"
        with open(output_file, 'r', encoding='utf-8') as f:
    ...
    def __getitem__(self, index):
        ...
        return image, multi_captions, captions,  self.img_ids[ann['id']], img_path, gpt_gen_caption


class cuhk_pre_train1(Dataset):
    def __init__(self, transform, image_root, max_words=72, prompt=''):
        '''
        image_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
...
class cuhk_pede_train(Dataset):
    def __init__(self, transform, datasets, max_words=72, prompt=''):
        '''
        image_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
        '''
        ...
        self.img_ids = {}
        n = 0
        for ann in self.datasets:
            img_id = ann[0]
            if img_id not in self.img_ids.keys():
                self.img_ids[img_id] = n
                n += 1

        self.num_classes = len(self.img_ids.keys())

    def __len__(self):
        return len(self.datasets)

    def __getitem__(self, index):
        idx, image_path, captions, gpt_captions = self.datasets[index]
        image = Image.open(image_path)
        image = self.transform(image)
        captions = self.prompt+pre_caption(captions, self.max_words)

        return image, captions, idx, gpt_captions
